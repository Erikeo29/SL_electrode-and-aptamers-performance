**Note de l'auteur** — *Ce projet a été conçu intégralement par l'auteur, depuis une page blanche jusqu'à sa mise en ligne. La création du contenu a été réalisée avec le support d'outils d'intelligence artificielle, en particulier pour la rédaction et la correction des codes et pour les recherches internet. Tous les résultats montrés dans ce projet sont issus de modèles physiques déterministes résolus par éléments finis (Firedrake). Ce travail est mis à disposition en open-source : il peut être librement copié, dupliqué et adapté à des fins d'apprentissage.*
...
La qualité du signal dépend de nombreux facteurs : état de surface de l'électrode (rugosité, composition, contamination), paramètres instrumentaux (fréquence SWV) et conditions de mesure (résistance de contact). Cette application regroupe **trois études paramétriques** simulées par éléments finis afin de quantifier l'influence de chaque facteur sur les métriques du voltammogramme :
...
Les résultats présentés proviennent de simulations par éléments finis résolues sous **Firedrake** avec le package **EchemFEM**. Le solveur résout l'équation de diffusion instationnaire dans le film d'aptamères, couplée à la cinétique de Butler-Volmer à la surface de l'électrode. L'extraction des métriques ($I_{peak}$, $E_{peak}$, FWHM, SNR) est réalisée par ajustement gaussien du pic SWV.
